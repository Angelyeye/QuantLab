...
def MCLPlotResultA2(pathnow, MCLPlotName, TimeAll, ReturnCum, ReturnWorth, MaxDfDrawValueNow, MaxDfDrawRatioNow, MCLInitialMoney, MCLFinalReturnValueAll, MCLRealYears, MCLYearReturnValue, MCLBestPercent, MCLBestValue, MCLMaxDrawValueAll, MCLFinalDrawValue, MCLMaxDrawRatioAll, MCLFinalDrawRatio, MCLFinalWorth, MCLYearReturnRatio, MCLFollowBeginTime, MCLFollowReturnValue, MCLFollowMaxDrawValue, MCLFollowMaxDrawRatio, MCLFollowYearReturnValue, MCLFollowBestValue):
    FollowBeginPos = TimeAll[TimeAll>=int(MCLFollowBeginTime)].index[0]
    TimeAllNormal = TimeAll.copy()
    TimeAllNormalFinal = [parse(str(TimeAllNormal[i])) for i in range(0,len(TimeAllNormal))]
    plt.rcParams['font.sans-serif']=['SimHei']                           
    plt.rcParams['axes.unicode_minus']=False                             
    plt.figure(figsize=(12,8))
    ax1 = plt.subplot(211)
    plt.plot(TimeAllNormalFinal[0:FollowBeginPos + 1], ReturnCum[0:FollowBeginPos + 1], c='black',lw = 2)
    plt.plot(TimeAllNormalFinal[FollowBeginPos:], ReturnCum[FollowBeginPos:], c='red',lw = 2)
    plt.xticks(color='blue',rotation=90)
    plt.grid(True)
    plt.axis('tight')
    plt.ylabel('资   产',size=15)
    ...
    plt.title(MCLPlotName + '资产曲线,初始资金'+str(MCLInitialMoney)+'万,最后利润为:'+str(round(MCLFinalReturnValueAll,2))+',交易年数:'+str(MCLRealYears)+'\n'+'年化收益金额为:'+str(round(MCLYearReturnValue,2))+'元,收益回撤比(%):'+str(MCLBestPercent)+',收益回撤比(金额):'+str(MCLBestValue),size=15) 
    ax2=plt.subplot(212)
    plt.plot(TimeAllNormalFinal[0:FollowBeginPos + 1], MaxDfDrawValueNow[0:FollowBeginPos + 1], c='black',lw = 2)
    plt.plot(TimeAllNormalFinal[FollowBeginPos:], MaxDfDrawValueNow[FollowBeginPos:], c='blue',lw = 2)
    plt.xticks(color='blue',rotation=90)
    plt.grid(True)
    plt.axis('tight')
    plt.xlabel('日   期',size=15)
    plt.ylabel('回   撤(金额)',size=15)
    plt.title(MCLPlotName+'金额回撤,最大回撤:'+str(round(MCLMaxDrawValueAll,2))+'元'+',最后回撤金额:'+str(round(MCLFinalDrawValue,2))+'元'+ '\n' + '外延收益:' + str(MCLFollowReturnValue) +'元,外延最大回撤:' + str(MCLFollowMaxDrawValue) + '元,外延年化收益:' + str(MCLFollowYearReturnValue) + ',外延收益回撤比:' + str(MCLFollowBestValue), size=15) 
    plt.xlim([TimeAllNormalFinal[0],TimeAllNormalFinal[-1]]) # 日期上下限
    ax2.xaxis.set_major_formatter(mdates.DateFormatter("%Y-%m-%d"))    
    plt.tight_layout()
    pathpicnow = os.path.join(pathnow,'1.' + MCLPlotName + '_资产曲线And金额回撤图.jpg')
    ...
    plt.axis('tight')
    plt.ylabel('资   产',size=15)
    plt.xlim([TimeAllNormalFinal[0], TimeAllNormalFinal[-1]]) 
    ax1.xaxis.set_major_formatter(mdates.DateFormatter("%Y-%m-%d"))    
    plt.title(MCLPlotName + '资产曲线,初始资金'+str(MCLInitialMoney)+'万,最后利润为:'+str(round(MCLFinalReturnValueAll,2))+',交易年数:'+str(MCLRealYears)+'\n'+'年化收益金额为:'+str(round(MCLYearReturnValue,2))+'元,收益回撤比(%):'+str(MCLBestPercent)+',收益回撤比(金额):'+str(MCLBestValue),size=15) 
    ax2=plt.subplot(212)
    plt.plot(TimeAllNormalFinal[0:FollowBeginPos + 1], MaxDfDrawRatioNow[0:FollowBeginPos + 1], c='black',lw = 2)
    plt.plot(TimeAllNormalFinal[FollowBeginPos:], MaxDfDrawRatioNow[FollowBeginPos:], c='blue',lw = 2)
    plt.xticks(color='blue',rotation=90)
    plt.grid(True)
    plt.axis('tight')
    plt.xlabel('日   期',size=15)
    plt.ylabel('回   撤(%)',size=15)
    plt.title(MCLPlotName+'百分比回撤,最大回撤比率:'+str(round(MCLMaxDrawRatioAll,2))+'%'+',最后回撤比例:'+str(round(MCLFinalDrawRatio,2))+'%'+ '\n' + '外延收益:' + str(MCLFollowReturnValue) +'元,外延最大回撤比率:' + str(MCLFollowMaxDrawRatio) + '%,外延年化收益:' + str(MCLFollowYearReturnValue) + ',外延收益回撤比:' + str(MCLFollowBestValue),size=15) 
    plt.xlim([TimeAllNormalFinal[0], TimeAllNormalFinal[-1]]) 
    ax2.xaxis.set_major_formatter(mdates.DateFormatter("%Y-%m-%d"))    
    plt.tight_layout()
    pathpicnow = os.path.join(pathnow, '2.' + MCLPlotName +'_资产曲线And百分比回撤图.jpg')
    ...
    plt.figure(figsize=(12,8))
    ax1=plt.subplot(211)
    plt.plot(TimeAllNormalFinal[0:FollowBeginPos + 1], ReturnWorth[0:FollowBeginPos + 1], c='black',lw = 2)
    plt.plot(TimeAllNormalFinal[FollowBeginPos:], ReturnWorth[FollowBeginPos:], c='red',lw = 2)
    plt.xticks(color='blue',rotation=90)
    plt.grid(True)
    plt.axis('tight')
    plt.ylabel('净   值',size=15)
...
def MCLEasyQuantBackTestCoverCount(MCLResultFilePath, MCLData, MCLNormalDataFinal, MCLStrategyName, MCLSymbolName, MCLPeriodName, MCLLots, MCLInitialMoney, MCLCostType, MCLMultiplier, MCLMinMove, MCLCostMoney, MCLCostPercent, MCLFollowBeginTime):
    ...
    if MCLFollowBeginTime == 0:
        MCLPlotResultA1(pathnow, MCLPlotName, TimeAll, ReturnCum, ReturnWorth, MaxDfDrawValueNow, MaxDfDrawRatioNow, MCLInitialMoney, MCLFinalReturnValueAll, MCLRealYears, MCLYearReturnValue, MCLBestPercent, MCLBestValue, MCLMaxDrawValueAll, MCLFinalDrawValue, MCLMaxDrawRatioAll, MCLFinalDrawRatio, MCLFinalWorth, MCLYearReturnRatio)
    else:
        FollowBeginNowThis = TimeAll[TimeAll>=int(MCLFollowBeginTime)].index[0]
        FollowEndNowThis = TimeAll[TimeAll>=TimeAll.iloc[-1]].index[0]
        MCLFollowReturnValue = round(ReturnCum[FollowEndNowThis] - ReturnCum[FollowBeginNowThis-1], 2)
        MCLFollowMaxDrawValue = round(min(MaxDfDrawValueNow[FollowBeginNowThis:FollowEndNowThis+1]), 2)
        MCLFollowMaxDrawRatio = round(min(MaxDfDrawRatioNow[FollowBeginNowThis:FollowEndNowThis+1]), 2)
        temp4 = str(MCLFollowBeginTime)
        temp5 = str(TimeAll.iloc[-1])
        temp6 = datetime.datetime(int(temp5[0:4]), int(temp5[4:6]), int(temp5[6:8])) - datetime.datetime(int(temp4[0:4]), int(temp4[4:6]), int(temp4[6:8]))
        MCLRealYears2 = round(temp6.days / 365, 2)  
        MCLFollowYearReturnValue = round(MCLFollowReturnValue / MCLRealYears2, 2)
        MCLFollowBestValue = round((MCLFollowYearReturnValue / abs(MCLFollowMaxDrawValue)), 4)
        MCLPlotResultA2(pathnow, MCLPlotName, TimeAll, ReturnCum, ReturnWorth, MaxDfDrawValueNow, MaxDfDrawRatioNow, MCLInitialMoney, MCLFinalReturnValueAll, MCLRealYears, MCLYearReturnValue, MCLBestPercent, MCLBestValue, MCLMaxDrawValueAll, MCLFinalDrawValue, MCLMaxDrawRatioAll, MCLFinalDrawRatio, MCLFinalWorth, MCLYearReturnRatio, MCLFollowBeginTime, MCLFollowReturnValue, MCLFollowMaxDrawValue, MCLFollowMaxDrawRatio, MCLFollowYearReturnValue, MCLFollowBestValue)
    return 0
...
def MCLDIYEasyQuantRun():
    ...
    MCLEasyQuantBackTestCoverCount(MCLResultFilePath, MCLData, MCLNormalDataFinal, MCLStrategyName, MCLSymbolName, MCLPeriodName, MCLLots, MCLInitialMoney, MCLCostType, MCLMultiplier, MCLMinMove, MCLCostMoney, MCLCostPercent, MCLFollowBeginTime)
    time_end = time.time()
